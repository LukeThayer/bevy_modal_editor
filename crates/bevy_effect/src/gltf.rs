//! GLTF source component + materializing systems.
//!
//! Moved here from `bevy_modal_editor::scene::gltf_source` because
//! `EffectAction::SpawnGltf` inserts [`GltfSource`] and relies on these
//! systems to materialize the model. The `#[type_path]` pins the original
//! reflected path so saved scenes keep loading. The editor re-exports these
//! items from their old location and keeps its own `SpawnGltfEvent` handler.
...
#[type_path = "bevy_modal_editor::scene::gltf_source"]
...
/// Registers [`GltfSource`] and the systems that materialize it (scene load on
/// add/change, cleanup on removal).
///
/// [`crate::EffectPlugin`] adds this automatically if not already present.
